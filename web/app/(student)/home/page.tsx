        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[2fr_1fr]">
          ...
          <div className="grid gap-4">
            <div className="relative rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm">
              ...
              <div className="text-xs text-slate-500">All available full-length practice tests</div>
              <div className="mt-4 pr-40 sm:pr-56 lg:pr-72">
                {loadingPractices ? (
                  <div className="text-sm text-slate-500">Loading practice tests...</div>
                ) : practiceError ? (
                  <div className="text-sm text-red-600">{practiceError}</div>
                ) : practiceTests.length === 0 ? (
                  <div className="text-sm text-slate-500">No practice tests yet.</div>
                ) : (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {practiceTests.map((p) => {
                      const resultEntries = Object.entries(p.attempt?.module_scores || {});
                        ...
                        >
                          ...
                          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                            {isLocked ? (
                              ...
                              <div>
                                <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-500">
                                  Submitted
                                </div>
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {resultEntries.map(([key, val]) => (
                                    <span
                                      key={key}
                                      className="rounded-full bg-white px-2 py-1 text-[11px] font-semibold text-slate-700 shadow-sm"
                                    >
                                      {key.toUpperCase()}: {val.correct}/{val.total}
                                    </span>
                                  ))}
                                </div>
                              </div>
                              ...
                              <div className="flex items-center justify-between">
                                <div>
                                  ...
                                  <div className="mt-1 text-sm font-semibold text-blue-600">Take a practice test</div>
                                </div>
                          ...
                          <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <button
                              ...
                              onClick={() => {
                                if (isLocked) return;
                                router.push(`/practice/modules/${p.id}?exam=sat`);
                              }}
                              className={`w-full rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] transition sm:w-auto ${
                                isLocked
                                  ? "cursor-not-allowed bg-slate-200 text-slate-500"
                                  : "bg-slate-900 text-white hover:bg-slate-800"
                              }`}
                            >
                              {hasResults ? "View Results" : "Start Practice"}
                            </button>
                            {!isLocked ? (
                              <div className="text-[11px] font-semibold text-slate-400">
                                {hasResults ? "See latest results" : "Jump in now"}
                              </div>
                            ) : null}
                          </div>
                        </div>
