...
function splitTopicTag(tag: string) {
  const raw = (tag || "").trim();
  if (!raw) return { topic: "", subtopic: "" };
  const parts = raw.split(" - ");
  if (parts.length === 1) return { topic: raw, subtopic: "" };
  return { topic: parts[0], subtopic: parts.slice(1).join(" - ") };
}

function buildTopicTag(topic: string, subtopic: string) {
  const base = topic.trim();
  if (!subtopic.trim()) return base;
  return `${base} - ${subtopic.trim()}`;
}

export default function EditModulePracticeQuestionPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/api/module-practice/questions/${questionId}/`, {
        ...
        const split = splitTopicTag(q.topic_tag || "");
        setTopic(split.topic);
        setSubtopic(split.subtopic);
        setStem(q.question_text ?? "");
        ...
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-white">
              Module practice - {subject}
            </div>
            <h1 className="text-3xl font-semibold text-slate-900">Edit question</h1>
            <p className="text-sm text-slate-600">Update stem, passage, answers, and metadata.</p>
          </div>
