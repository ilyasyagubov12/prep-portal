...
export default function NewModulePracticeQuestionPage() {
  const params = useParams<{ subject: string; practiceId: string }>();
  const subject = (params.subject || "").toLowerCase() as "verbal" | "math";
  const practiceId = params.practiceId;
  const router = useRouter();
  const search = useSearchParams();
  const moduleIndexParam = search.get("module") ?? "";
  ...
  async function handleSubmit() {
    ...
    if (!practiceId) return setError("Missing practice id");
    if (!Number.isFinite(moduleIndex) || moduleIndex < 1) return setError("Missing module index");
