...
export async function POST(req: Request) {
  try {
    ...
    const assignment_body = typeof body?.body !== "undefined" ? body.body : undefined;
    const due_at = body?.due_at ?? undefined;
    const max_score = typeof body?.max_score === "number" ? body.max_score : undefined;
    const max_submissions = typeof body?.max_submissions === "number" ? body.max_submissions : undefined;
    ...
    if (assignment_body !== undefined) patch.body = assignment_body;
    if (due_at !== undefined) patch.due_at = due_at;
    if (max_score !== undefined) patch.max_score = max_score;
    if (max_submissions !== undefined) patch.max_submissions = max_submissions;
